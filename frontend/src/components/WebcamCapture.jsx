...
const WebcamCapture = ({ onConnectionStatus, onPoseDataUpdate }) => {
    ...
    const { sendMessage, lastMessage, connectionStatus } = useWebSocket('ws://localhost:8000/ws/pose');
    ...
    useEffect(() => {
        if (!isCameraReady || connectionStatus !== 'Open') return;
        ...
        const processFrame = (timestamp) => {
            if (timestamp - lastTime >= INTERVAL) {
                lastTime = timestamp;
                setFps(Math.round(1000 / (timestamp - lastTime + INTERVAL))); // Rough FPS calc
    ...
    }, [isCameraReady, connectionStatus, sendMessage]);
        ...
        <div className="relative w-full max-w-2xl mx-auto rounded-xl overflow-hidden shadow-2xl bg-black border border-gray-800">
            ...
            {poseData && (
                <div className="absolute inset-0 pointer-events-none transform scale-x-[-1]">
                    <SkeletonOverlay poseData={poseData} width={640} height={480} />
                </div>
            )}
